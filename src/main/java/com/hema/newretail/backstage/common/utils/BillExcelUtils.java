...
import com.hema.newretail.backstage.entry.orderentry.OrderExcleEntry;
...
public class BillExcelUtils {
    //根据指定的excel模板导出数据
    public static void exportExcel(String orderSrcFileName,List<OrderExcleEntry> data,OutputStream fos,String time) throws Exception {
        //创建Excel文件的输入流对象
        InputStream in = BillExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+orderSrcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            OrderExcleEntry order = data.get(i);
            //创建新的一行，并复制上面的样式
            ...
            newRow.getCell(1).setCellValue(order.getBillId());
            newRow.getCell(2).setCellValue(order.getBillTime());
            newRow.getCell(3).setCellValue(order.getNickName());
            newRow.getCell(4).setCellValue(order.getDetails());
            newRow.getCell(5).setCellValue(order.getAmt());
            newRow.getCell(6).setCellValue(order.getBillStatus());
            newRow.getCell(7).setCellValue(order.getPaymentType());
            newRow.getCell(8).setCellValue(order.getPaySerialNumbers());
            newRow.getCell(9).setCellValue(order.getClearStatus());
            newRow.getCell(10).setCellValue(order.getRemark());
        }
